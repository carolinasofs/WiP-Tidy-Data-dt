...
## Countries with the highest percentage of women in parliament by Year
...
cWP[Year %in% c(1990, 2018) & !is.na(Continent)][
  order(Year, -pctWiP), head(.SD, 1), by = .(Year, Continent)][ 
...
```{r}
dWP <- cWP[
  order(Country, Year), .SD[c(1, .N)], by = Country][,
...
Now, plot the trend lines for countries with at least a 5% decline 
...
Now, for countries with at least a 10% decline

```{r}
dclpct <-  unique(dWP[!is.na(Continent) & pctDiff <= -10]$Country)
WP[Country %in% dclpct] %>% 
  ggplot(aes(Year, pctWiP, colour = Country)) +
  geom_line() +
  geom_point() +
  ...
  scale_y_continuous(limits = c(0, 40),
  breaks = seq(0, 40, by = 10)) +
  ggtitle("Women in Parliament: Decline >= 10%") + 
  ylab("% Women in Parliament")
```

Now, for countries with at least a 1% decline

```{r}
dclpct <-  unique(dWP[!is.na(Continent) & pctDiff <= -15]$Country)
WP[Country %in% dclpct] %>% 
  ggplot(aes(Year, pctWiP, colour = Country)) +
  geom_line() +
  geom_point() +
  ...
  scale_y_continuous(limits = c(0, 40),
  breaks = seq(0, 40, by = 10)) +
  ggtitle("Women in Parliament: Decline >= 15%") + 
  ylab("% Women in Parliament")
```

# Ranked status - Global Ranks by Year
...
## Global Ranking - Portugal and China
```{r}
...
## Portugal's Ranking in Europe and it's plot 
...
## China's Ranking in Asia and it's plot 
...
```{r}
cWP[Country %in% c("China", "Japan", "Thailand", "Bangladesh", "Vietnam", "Indonesia")] %>%
  ggplot(aes(Year, RankC, colour = Country)) +
  geom_line() +
  geom_point() +
  scale_x_continuous(breaks = seq(1990, 2020, 5)) +
...
China has one of the lowest rankings comparing to other countries in Asia, out of the ones considered, only Vietnam has a worst ranking than China. 
